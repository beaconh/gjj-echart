/**
 * 公历转农历（1900-2100）
 * 基于 Jea杨 calendar.js 精简，仅保留 solar2lunar 所需逻辑
 */
(function (global) {
  const lunarInfo = [
  ...
  function getTerm(y, n) {
    ...
    const info = [0, 5, 10, 15, 20, 25].map((s) => parseInt("0x" + table.substr(s, 5)).toString());
    const calday = [];
    info.forEach((chunk) => {
      calday.push(chunk.substr(0, 1), chunk.substr(1, 2), chunk.substr(3, 1), chunk.substr(4, 2));
    });
  ...
  function solar2lunar(y, m, d) {
    ...
    let term = null;
    if (firstNode === d) term = solarTerm[m * 2 - 2];
    if (secondNode === d) term = solarTerm[m * 2 - 1];

    const IMonthCn = (isLeap ? "闰" : "") + toChinaMonth(month);
    const IDayCn = toChinaDay(day);
    let label = IDayCn;
    if (day === 1) label = IMonthCn;
    if (term) label = term;
  ...
  function lunarLabel(dateKey) {
    if (!dateKey) return "";
    if (cache[dateKey] !== undefined) return cache[dateKey];
    const [y, m, d] = dateKey.split("-").map(Number);
    const lunar = solar2lunar(y, m, d);
    cache[dateKey] = lunar ? lunar.label : "";
    return cache[dateKey];
  }

  global.LunarCalendar = { solar2lunar, lunarLabel };
